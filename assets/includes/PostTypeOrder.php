...
class PostTypeOrder
{
    ...
    */
    ...
    {
        load_muplugin_textdomain('pt-order', basename(TW_PTO_PLUGIN_PATH) . '/assets/languages/');
    }
    ...
    */
    public function addScripts()
    {
        wp_enqueue_script(
            'tp-pt-order',
            TW_PTO_PLUGIN_PATH . '/assets/scripts/admin.js',
            array('jquery', 'jquery-ui-sortable')
        );
        wp_enqueue_style('tp-pt-order', TW_PTO_PLUGIN_PATH . '/assets/styles/admin.css');
    }
    ...
    */
    public function displayPosts($args)
    {
        $defaults = array(
         'posts_per_page' => -1
        );
    ...
    */
    public function getPosts($term, $taxonomy, $post_type)
    {
       ...
       */
        if (is_array($pt_order[$term->term_id])) {
            foreach ($pt_order[$term->term_id] as $key => $term_id) {
                if (!in_array($term_id, $all_posts_ids)) {
                    unset($pt_order[$term->term_id][$key]);
                }
            }
        }
